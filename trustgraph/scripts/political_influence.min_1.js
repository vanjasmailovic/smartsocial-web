/*! political_influence 04-11-2015 */
function log(a) {}
...
function initialize() {
    if (totalContributions = 0, renderLinks = [], cands = [], pacs = [], contr = [], "house" == office) {
        ...
        }), log("totalCandAmount=" + e), pacs = pacsHouse, c_house.forEach(function(a) {
            contr.push(a)
        })
...
function node_onMouseOver(a, b) {
    ...
    if (c + 250 > window.innerWidth && (c = d3.event.pageX - 280), "CAND" == b) {
        if (a.depth < 2) return;
        toolTip.transition().duration(200).style("opacity", ".9"), header1.text("Facebook friend"), header.text(a.CAND_NAME), header2.text("Total Recieved: " + formatCurrency(Number(a.Amount))), toolTip.style("left", c + "px").style("top", d3.event.pageY - 150 + "px").style("height", "100px"), highlightLinks(a, !0)
...
var svg = d3.select(document.getElementById("svgDiv")).style("width", 2 * outerRadius + 200 + "px").style("height", 2 * outerRadius + 200 + "px").append("svg").attr("id", "svg").style("width", 2 * outerRadius + 200 + "px").style("height", 2 * outerRadius + 200 + "px"),
    ...
    h_others = [],
    house = [];
s_dems = [], s_reps = [], s_others = [], senate = [], total_hDems = 0, total_sDems = 0, total_hReps = 0, total_sReps = 0, total_hOthers = 0, total_sOthers = 0, contributions = [], c_senate = [], c_house = [], pacs = [], pacsById = {}, chordsById = {}, nodesById = {}, chordCount = 20, pText = null, pChords = null, nodes = [], renderLinks = [], colorByName = {}, totalContributions = 0, delay = 2;
